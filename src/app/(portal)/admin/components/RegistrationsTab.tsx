...
type RegistrationsTabProps = Pick<AdminPanel,
  ...
  'actionLoading' | 'handleConfirmAndInvite' | 'handleUpdateRegistrationStatus' | 'cohorts' | 'fetchAll'
>

export default function RegistrationsTab({ registrations, expandedRegistration, setExpandedRegistration, registrationFilter, setRegistrationFilter, actionLoading, handleConfirmAndInvite, handleUpdateRegistrationStatus, cohorts, fetchAll }: RegistrationsTabProps) {
  const [editingRegId, setEditingRegId] = useState<string | null>(null)
  const [editReg, setEditReg] = useState({ full_name: '', email: '', phone: '', organization: '', cohort_id: '' })
  ...
  const handleSaveEdit = async (regId: string) => {
    if (!editReg.full_name.trim() || !editReg.email.trim()) return
    setSavingEdit(true)
    const { error } = await supabase.from('registrations').update({
      full_name: editReg.full_name,
      email: editReg.email,
      phone: editReg.phone || null,
      organization: editReg.organization || null,
      cohort_id: editReg.cohort_id || null,
    }).eq('id', regId)
    if (!error) {
        ...
        <div style={{ ...cardStyle, padding: 0, overflow: 'hidden' }}>
          {registrations
            ...
            .map(reg => {
              ...
              const cohortOptions = cohorts.filter(c => c.program_id === reg.program_id)
              return (
              <div key={reg.id}>
                <div
                  ...
                  <div style={{ padding: '1.25rem 1.5rem', background: 'var(--paper)', borderBottom: '1px solid var(--mist)' }}>
                      ...
                      <div style={{ marginBottom: '1.25rem' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                          <div>
                            ...
                            <input value={editReg.full_name} onChange={e => setEditReg({ ...editReg, full_name: e.target.value })} style={inputStyle} />
                          </div>
                          <div>
                            <label style={labelStyle}>Email</label>
                            <input type="email" value={editReg.email} onChange={e => setEditReg({ ...editReg, email: e.target.value })} style={inputStyle} />
                          </div>
                          <div>
                            <label style={labelStyle}>Phone</label>
                            <input value={editReg.phone} onChange={e => setEditReg({ ...editReg, phone: e.target.value })} style={inputStyle} />
                          </div>
                          <div>
                            <label style={labelStyle}>Organization</label>
                            <input value={editReg.organization} onChange={e => setEditReg({ ...editReg, organization: e.target.value })} style={inputStyle} />
                          </div>
                          <div style={{ gridColumn: '1 / -1' }}>
                            <label style={labelStyle}>Cohort</label>
                            <select value={editReg.cohort_id} onChange={e => setEditReg({ ...editReg, cohort_id: e.target.value })} style={inputStyle}>
                        ...
                        <div style={{ display: 'flex', gap: '0.75rem' }}>
                          <button onClick={() => handleSaveEdit(reg.id)} disabled={savingEdit} className="btn btn-primary" style={{ fontSize: '0.8rem', padding: '0.55rem 1.1rem' }}>
                            {savingEdit ? 'Saving...' : 'Save'}
                          </button>
                          <button onClick={() => setEditingRegId(null)} style={{ background: 'none', border: '1px solid rgba(0,23,55,0.15)', color: 'var(--navy)', borderRadius: '2px', padding: '0.55rem 1.1rem', fontSize: '0.8rem', cursor: 'pointer' }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
                    ...
                    {!isEditing && (
                      <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                        ...
                        <button onClick={() => startEdit(reg)} style={{ background: 'none', border: '1px solid rgba(200,136,32,0.4)', color: 'var(--gold)', borderRadius: '2px', padding: '0.55rem 1.1rem', fontSize: '0.8rem', cursor: 'pointer' }}>Edit</button>
                        {reg.status !== 'waitlist' && (
